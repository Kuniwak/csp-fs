...
let testCases: obj[] list =
    [ [| { Input = "(unwind P)"
      ...
      [| { Input = "(prefixRecv y x stop)"
           ...
           Line = __LINE__ } |]
      [| { Input = "(in stop skip (unwind P))"
           Expected = intCh (intCh (stop "1") (skip "1") "1") (unwind "P" [] "1") "1"
           Line = __LINE__ } |]
      [| { Input = "(ex stop skip (unwind P))"
           Expected = extCh (extCh (stop "1") (skip "1") "1") (unwind "P" [] "1") "1"
           Line = __LINE__ } |]
      [| { Input = "(seq stop skip (unwind P))"
           Expected = seq (seq (stop "1") (skip "1") "1") (unwind "P" [] "1") "1"
           Line = __LINE__ } |]
      ...
      [| { Input = "(para stop x stop)"
           ...
           Line = __LINE__ } |]
      [| { Input = "(interleave stop skip (unwind P))"
           Expected = interleave (interleave (stop "1") (skip "1") "1") (unwind "P" [] "1") "1"
           Line = __LINE__ } |]
      [| { Input = "(hide stop x)"
