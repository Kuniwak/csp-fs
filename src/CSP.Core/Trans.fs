...
let rec trans
    ...
    | Stop _ -> []
    | Prefix(env, expr, s) -> [ (Vis(eval env expr), s) ]
    ...
    | If(env, expr, s1, s2) ->
        match eval env expr with
        | VBool true -> trans m env s1
        | VBool false -> trans m env s2
    ...
    | Match(env, expr, sm, ds) ->
        ...
        | VUnion(c, v) ->
            ...
            | Some(var, p2) -> trans m env (bind var v p2)
            | None ->
                match ds with
                | Some(Some var, p2) -> trans m env (bind var (VUnion(c, v)) p2)
                | Some(None, p2) -> trans m env p2
