...
let infer
    ...
    : Result<(ProcMap<TypeCstr> * Proc<TypeCstr>) * State, TypeError> =
        ...
        |> Result.bind (fun (pm', s) -> ProcTypeInference.infer pm um cm tcenv p s |> Result.map (fun (p, s) -> ((pm', p), s)))
...
let typeCheck (um: UnionMap) (cm: CtorMap) (genv: Env) (pm: ProcMap<unit>) (p: Proc<unit>) : Result<ProcMap<Type> * Proc<Type>, TypeError> =
    typeEnv um cm genv init
    |> Result.bind (fun (tcenv, s) ->
        infer um cm tcenv pm p s
        |> Result.bind postProcess)
